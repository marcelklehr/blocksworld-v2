...
plotTrials <- function(dat, trials="all", min=0, responses="all"){
  ids = dat$id %>% unique()
  n = ids %>% length();
  brks=seq(0, 1, by=0.1)
...
```{r independent-trials, echo=FALSE}
...
literals <- utterances %>% filter(!str_detect(response, "and|neither|but|if")) %>% 
  pull(response)
df <- test.standardized %>% group_by(id)
df %>% filter(str_detect(id, "if1")) %>%
  plotTrials(trials="", min=0, responses=literals)
