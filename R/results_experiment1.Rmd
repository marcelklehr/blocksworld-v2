...
data.orig <- data.from_rds %>% mutate(response=r_orig) %>% select(-r_norm, -r_orig)
data.norm <- data.from_rds %>% mutate(response=r_norm) %>% select(-r_norm, -r_orig)
# choose wether normalized or unnormalized values shall be plotted
data.plot <- data.orig


stimuli <- data.from_rds %>% pull(id) %>% unique()
N_participants <- data.from_rds %>% pull(prolific_id) %>% unique() %>% length()
...
dataIndependent = function(data) {
  ...
  df <- data %>% mutate(prod_ac=p_a*p_c, p=ac-prod_ac, p_key = "diff_ac_prod_ac") %>% 
    select(prolific_id, id, p, p_key)
    
  labels <- c(diff_ac_prod_ac="P(A,C)-P(A)xP(C)")
              # , ac="P(Blue,Green)", prod_ac="P(Blue) * P(Green)")
...
```{r}
plotProbs <- function(df, xlab, limits_x = c(-0.2, 1.2), breaks_x = c(0, 0.25, 0.5, 0.75, 1)){
  ...
  for(s in df$stimuli) {
    ...
    p <- df.s  %>%
      ...
      geom_vline(data = means, aes(xintercept=m, linetype=p_key), color="orange", size=0.5) +
      geom_point(data=means,  mapping=aes(x=med, y=factor(0)), shape = 5, size=2, alpha=0.5) +
      # scale_shape_identity(guide = "legend", aesthetics="shape") + 
      # geom_violin(alpha=0.5) +
      labs(x=xlab, y="") + 
      theme_bw() +
      ggtitle(s) +
      theme(legend.position = "none",
            ...
            panel.spacing = unit(2, "lines"))
      # guides(color = FALSE, linetype=guide_legend(title="mean value")) 
    print(p)
...
plotProbs(df, "P(A,C)-P(A)xP(C)", limits_x = c(-0.5, 0.5), breaks_x = seq(-0.5, 0.5, 0.25))
```
...
plotProbs(df, "delta p: P(C|A)-P(C)",
          limits_x = c(min, max),
          breaks_x = c(0, seq(round(min, 1), round(max, 1) + 0.05, 0.25)))
```
...
min <- df$data$p %>% min() - 0.1
max <-  df$data$p %>% max() + 0.1
plotProbs(df, "delta P* = (P(C|A)-P(C))/(P(¬A,¬C)", 
          limits_x = c(min, max),
          breaks_x = c(seq(round(min, 1), round(max, 1) + 0.05, 0.25)))
```
