...
fn <- "exp1_tidy.rds"
# fn <- "exp1_tidy_steepness_noticed.rds"
...
for (pa in seq(1, 10)){
  p <- df  %>%
    ...
    geom_jitter(height = 0.1, alpha=0.5, aes(colour = cluster)) +
    geom_point(mapping=aes(x=mu, y=factor(0)),  shape=24, color = "red") +
...
plotProbs(df, "delta p: P(C|A)-P(C)", "expected interpretation: A->C",
          limits_x = c(min, max),
...
min <- df$data$p %>% min() - 0.1
max <-  df$data$p %>% max() + 0.1
plotProbs(df, "delta P* = (P(C|A)-P(C))/(P(¬A,¬C)", "other interpretations",
          limits_x = c(min, max),
