...
as_a_implies_c_cause <- c('a_iff_c_lh', 'a_iff_c_hl', 'a_implies_c_hl')
ac_cause <- df %>% filter(id %in% as_a_implies_c_cause) %>%
  add_column(kind = "cause")

as_a_implies_c_diagnostic <- c('a_iff_c_lu', 'a_iff_c_ul', 'a_implies_c_ul')
...
```{r, echo=FALSE}
plotProbs = function(data){
  stimuli <- data %>% pull(id) %>% unique()
  labels <- c(p_a="P(Blue falls)", p_c="P(Green falls)",
            p_c_given_a="P(Green| Blue)",
            p_a_given_c="P(Blue| Green)")
  df <- data %>%
    select(prolific_id, id, p_a, p_c, p_c_given_a, p_a_given_c) %>% 
    pivot_longer(cols=c(p_a, p_c, p_c_given_a, p_a_given_c),
                 names_to = "p_key", values_to = "p") %>% 
    mutate(p_key=factor(p_key,levels=c("p_a","p_c","p_a_given_c", "p_c_given_a")))
  ...
  for(s in stimuli) {
    ...
    p <- df.s  %>%
      ...
      labs(x="estimates probability", y="") + 
      theme_classic() +
      facet_wrap(~p_key, labeller = labeller(p_key=labels)) + 
      ggtitle(s) +
      theme(legend.position = "right",
            axis.text.y=element_blank(),
            axis.ticks.y =element_blank(),
...
```{r}
plotProbs(bind_rows(ac_cause, ac_diagnostic))
```
