...
dataOthers <- function(data){
  ...
  df <- data %>%
    ...
    select(prolific_id, id, prod_ac, ac, p_a_given_c, p_a_given_nc,
           p_c_given_a, p_c_given_na) %>%
    pivot_longer(cols=c(prod_ac, ac, p_a_given_c, p_a_given_nc, p_c_given_a,
                        p_c_given_na),
                 names_to = "p_key", values_to = "p") %>% 
    mutate(p_key=factor(p_key, levels=c("prod_ac", "ac",
