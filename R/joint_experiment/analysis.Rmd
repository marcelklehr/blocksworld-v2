...
# Data Test Trials
prior.test = data.prior.norm %>% filter(id == "ind2")
...
df = bind_rows(df.bad, df.good) %>% select(-RT, -QUD, -group, -custom_response, -utterance)
df %>% 
...
```{r, quality-checks, fig.height=12}
df.quality = left_join(df.understood_rel, data.quality %>% rename(prolific_id=comparator),
               by=c("prolific_id", "id"))
...
p <- data.quality %>%
  # df.quality %>% 
  ggplot(aes(x=question,  y=mean_diff, colour=question)) + 
    facet_wrap(~id, scales="free") +
    geom_boxplot(aes(colour=question), outlier.shape=NA) +
    geom_jitter(height=0, width=0.05, alpha=0.4, size=0.5) + 
